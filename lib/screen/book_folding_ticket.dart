...
class BookFoldingTicket extends StatefulWidget {
  ...
  BookFoldingTicket({this.duration, required this.entries, this.isOpen = false, this.onClick});

  @override
  _BookFoldingTicketState createState() => _BookFoldingTicketState();
}

class _BookFoldingTicketState extends State<BookFoldingTicket> with SingleTickerProviderStateMixin {
  late List<FoldEntry> _entries = widget.entries;
  late AnimationController _controller = AnimationController(vsync: this);
  double _ratio = 0.0;

  double get openHeight => _entries.fold<double>(0.0, (val, o) => val + o.height) + BookFoldingTicket.padding * 2;

  double get closedHeight => _entries[0].height + BookFoldingTicket.padding * 2;
  ...
  @override
  void didUpdateWidget(BookFoldingTicket oldWidget) {
  ...
  Widget build(BuildContext context) {
    return Container(
      padding: EdgeInsets.all(BookFoldingTicket.padding),
